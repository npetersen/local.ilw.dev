...
<div class="gift-certificate">
	...
	<h3><?php _e( "Gift Membership Recipient's details", 'ignitewoo_gift_certs' ) ?></h3>
	...
	<p><?php _e( 'Leave the recipient name and email address blank to receive the gift membership yourself. Or, enter details below to send the gift membership(s) to someone else.', 'ignitewoo_gift_certs' ) ?></p>
	...
	<?php foreach ( $woocommerce->cart->cart_contents as $prod ) { ?>
		...
		<?php for ( $i = 0; $i < $prod['quantity']; $i++ ) { ?>
			...
			else if ( !empty( $prod['variation_id'] ) )
			...
			<div class="gift_cert_field_wrapper">
				...
				<?php /** DO NOT REMOVE ANY OF THESE FIELDS OR THE PLUGIN WILL EMAIL CERTIFICATES CORRECTLY ! */ ?>
